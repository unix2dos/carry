...
$('add').addEventListener('click',()=>showDialog('关联已有应用','内部版先关联已有 Railway 或 Vercel 项目与 Neon 资源。\n\n在已有 Agent 中选择项目，让它使用随源码提供的 ship Skill，核对资源归属后运行 register。\n\n首次创建云资源、通用资源接管和数据库迁移尚未接入这一版。'));
async function load(){
 ...
 for(const view of views){
  ...
  const provider=p.provider==='vercel'?'Vercel':'Railway';
  card.querySelector('.plan').textContent=o?provider+' '+(o.compute_plan||o.railway_plan||'待确认')+' · Neon '+o.neon_plan:'尚未读取资源状态';
