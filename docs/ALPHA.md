...
开发分支提供 `secret apply NAME KEY` 与 `secret reconcile NAME KEY` 的 Vercel 实现，真实写入尚未通过验收。`apply` 是明确的云端配置写入，需要已授权的个人非商业 Hobby 项目；`reconcile` 只核对上次标记。写入结果未知时保持阻止，不重复发送。已确认写入之后仍会比对远端变量 ID、更新时间和标记；这些只能证明记录一致，不能证明应用运行时实际使用了哪一数据库。参见 [当前接入问题](../validation/VERCEL-RESULTS.md)。
