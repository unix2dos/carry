...
1. Run `ship list` and select the project matching the user's request. For a new binding, establish its source directory and exact resource ownership before `register`. Keep cloud credentials in the official CLI stores; resource IDs belong in the binding, credentials do not.
2. Run `ship status NAME`. Report the observed account plan as Trial, Free or unverified. Trial is a time-limited test path, not proof of a formally validated Free combination. The executable performs fresh ownership and billing checks before each publish.
